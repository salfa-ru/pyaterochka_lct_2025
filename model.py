...
def load_model_and_tokenizer(model_dir='./best_saved_model_0.9'):
...
def process_text(text: str):
    ...
    mask = [1 if '##' not in tokenizer.convert_ids_to_tokens(i) else 0 for i in input_ids[1:-1].tolist()]
    ...
    for match in re.finditer(r'\S+', text):
        ...
        end_index = match.end()  # end() is exclusive, so subtract 1
